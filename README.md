...
- **Matching keyboard controls:** use arrow keys or `WASD` to drive, press `Space` repeatedly for the saber combo, and hold `Q` to charge the virtual pan-tilt training laser on the website, iPad/iPhone with a hardware keyboard, and Vision Pro with a connected keyboard.
- **Circuit School and Droid Workshop:** open the 80-build Circuit Quest from the Learn tab, watch completed tutorial ranges accumulate into tread, torso, camera/network, compute, arm, and commissioned robot sections, then choose a finish, housing material, and housing style. A checked Droid Code moves the profile between the website, iPhone, iPad, and Apple Vision Pro without an account.
