...
- **Matching keyboard controls:** use arrow keys or `WASD` to drive, `F` to move the flipper forward for a ledge climb, `B` to rotate it rearward for stable travel, `Space` for the saber combo, `Q` to charge the virtual pan-tilt training laser, and `E` to trigger the short bubble-shield window on iPad/iPhone with a hardware keyboard and Vision Pro with a connected keyboard.
